...
import { Typography, IconButton, ListItem, ListItemAvatar, ListItemText, Tooltip } from '@material-ui/core'
    ...
    return (
      <div className={classes.popoverContainer} onMouseLeave={() => handleClose()}>
        ...
        <OverlayScrollbarsComponent className={`${classes.list} ${type === DARK ? 'os-theme-light' : 'os-theme-dark'}`}>
          <IconButton className={classes.closeIcon} size="small" onClick={() => handleClose()}>
            <CloseIcon />
          </IconButton>
          {allUsers.map((userClass: any, index: number) => {
            return (
              <React.Fragment>
                ...
                {userClass &&
                  userClass.map((user: any, index: number) => {
  ...
  const Avatars = () => {
    ...
    return (
      <div onMouseEnter={handlePopoverOpen} onMouseLeave={handlePopoverClose}>
        <AvatarGroup max={5}>
          {allUsers.map((userClass: any, index: number) => {
            return (
              userClass &&
              userClass.map((user: any, index: number) => {
                return <Avatar src={getImageSrcById(user)} />
              })
            )
          })}
        </AvatarGroup>
        ...
        >
          <UsersInformationList handleClose={handlePopoverClose} />
        </Popover>
      </div>
    )
  }
    ...
    <div className={classes.root}>
      <div className={`${classes.flexGrow} ${classes.row}`}>
        <Tooltip
          ...
          title={<Typography color="textPrimary">{project.description}</Typography>}
        >
