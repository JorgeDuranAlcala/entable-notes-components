...
export const GlobalContext: IGlobalContext = {
  ...
  setThemeZoom: (themeZoom: ThemeZoom) => {
    ;(async () => localStorage && localStorage.setItem(THEME_ZOOM, JSON.stringify(themeZoom)))()
    GlobalContext.themeZoom = themeZoom
  },
  setSettings: (settings: Settings) => {
    ;(async () => localStorage && localStorage.setItem(SETTINGS, JSON.stringify(settings)))()
    GlobalContext.settings = settings
  },
  setCurrentBuffer: (buffer: Buffer) => {
    ;(async () => localStorage && localStorage.setItem(CURRENT_BUFFER, JSON.stringify(buffer)))()
    GlobalContext.currentBuffer = buffer
  },
  setBuffers: (buffers: Buffer[]) => {
    ;(async () => localStorage && localStorage.setItem(BUFFERS, JSON.stringify(buffers)))()
