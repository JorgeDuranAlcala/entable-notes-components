...
export function getContrastText(background: string, contrastThreshold = CONTRAST_TRESHOLD, warnings = true) {
  logWarnings &&
    warnings &&
    console.warn(background, `Material-UI: missing background argument in getContrastText(${background}).`)

  const contrastText = getContrastRatio(background, DARK) >= contrastThreshold ? DARK : WHITE
