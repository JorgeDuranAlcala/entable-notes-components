...
export function sortRows(array: any, cmp: any) {
  ...
  stabilizedThis.sort((a: any, b: any) => {
    ...
    Object.keys(a[0]).forEach((key: any) => {
      firstObj[key] = a[0][key].value
    })

    const secondObj = { ...Object.keys(b[0]) }
    Object.keys(b[0]).forEach((key: any) => {
      secondObj[key] = b[0][key].value
    })
...
export function getGridTemplateColumns(columns: Column[], defaultWidth = 3) {
  let minMaxColumns = ''
  columns.forEach(col => {
    const frac = !col.width ? 1 : (Math.round((col.width as number) / defaultWidth) * 100) / 100
...
export function isCellInRange(range: Range, rowIdx: number, colIdx: number) {
  ...
  return colIdx <= fromColIdx && colIdx >= toColIdx && rowIdx <= fromRowIdx && rowIdx >= toRowIdx
}
